...
class ACmix_Block(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_att=7, head=4, kernel_conv=3, stride=1, dilation=1):
        super(ACmix_Block, self).__init__()
...
"""
SENSORNET with ACmix block
"""


class SensorNet_ACmix(nn.Module):

    def __init__(self, in_planes, duration_window, duration_overlap, fs):

        super(SensorNet_ACmix, self).__init__()
        self.in_planes = in_planes
        ...
        self.resize = torchvision.transforms.Resize((48, 48), interpolation=InterpolationMode.BILINEAR)

        self.attncnn1 = ACmix_Block(self.in_planes, 64)
        self.attncnn2 = ACmix_Block(64, 128, stride=2)
        self.attncnn3 = ACmix_Block(128, 256, stride=2)

        self.norm1 = nn.LayerNorm([64, 48, 48])
        self.norm2 = nn.LayerNorm([128, 24, 24])
        self.norm3 = nn.LayerNorm([256, 12, 12])
    ...
    def forward(self, x):
        ...
        x = self.relu_FC0(self.norm1(self.attncnn11(x)))
        x = self.relu_FC0(self.norm2(self.attncnn2(x)))

        """
        last layer: global feature extraction in Fig.2 *
        """
        x = self.ap3(self.relu_FC0(self.norm3(self.attncnn3(x))))

        """
        Head-importance Learning in Fig. 1       
        """
